...
exports.notifyWeatherShare = exports.cleanupUserData = exports.initializeUserProfile = exports.autoLogMeetings = exports.notifyFriendRequestAccepted = exports.notifyFriendRequest = exports.resolveSOSAlert = exports.broadcastSOSAlert = exports.handleLocationUpdate = void 0;
 ...
 */
exports.handleLocationUpdate = functions.firestore
    ...
    try {
        ...
        for (const friendId of friendIds) {
            ...
            if (!friendLocSnapshot.exists)
                continue;
            const friendLocData = friendLocSnapshot.data();
            const friendCoords = friendLocData?.coordinates;
            const distance = calculateDistance(userCoords.latitude, userCoords.longitude, friendCoords.latitude, friendCoords.longitude);
            ...
            if (distance <= PROXIMITY_THRESHOLD) {
                ...
                if (!userGhost) {
                    await sendNotification(friendId, '📍 Nearby Alert!', `${userName} is nearby (${Math.round(distance)}m away)`, { type: 'nearby', userId, distance: distance.toString() });
                }
                if (!friendGhost) {
                    await sendNotification(userId, '📍 Nearby Alert!', `${friendName} is nearby (${Math.round(distance)}m away)`, { type: 'nearby', userId: friendId, distance: distance.toString() });
                }
            }
        }
    }
...
exports.broadcastSOSAlert = functions.firestore
    ...
    try {
        ...
        const notificationPromises = friendIds.map((friendId) => sendNotification(friendId, '🆘 Emergency SOS Alert!', `${userName} sent an emergency SOS alert`, {
            type: 'sos',
            sosId,
            userId,
 ...
 */
exports.resolveSOSAlert = functions.firestore
    .document('sos_alerts/{sosId}')
    ...
    try {
        ...
        const notificationPromises = (recipients || []).map((friendId) => sendNotification(friendId, '✅ SOS Resolved', `${userName}'s SOS alert has been resolved`, { type: 'sos_resolved', sosId }));
        await Promise.all(notificationPromises);
...
exports.notifyFriendRequest = functions.firestore
    ...
    try {
        ...
        await sendNotification(friendId, '👋 Friend Request', `${userName} sent you a friend request`, { type: 'friend_request', userId });
 ...
 */
exports.notifyFriendRequestAccepted = functions.firestore
    .document('friends/{friendRequestId}')
    .onUpdate(async (change, context) => {
    const before = change.before.data();
    const after = change.after.data();
    ...
    try {
        ...
        await sendNotification(userId, '✅ Friend Request Accepted', `${friendName} accepted your friend request`, { type: 'friend_accepted', friendId });
 ...
 */
exports.autoLogMeetings = functions.pubsub
    ...
    try {
        ...
        for (let i = 0; i < userIds.length; i++) {
            for (let j = i + 1; j < userIds.length; j++) {
                ...
                if (!loc1 || !loc2)
                    continue;
                const distance = calculateDistance(loc1.coordinates.latitude, loc1.coordinates.longitude, loc2.coordinates.latitude, loc2.coordinates.longitude);
 ...
 */
exports.notifyWeatherShare = functions.firestore
    ...
    try {
        ...
        if (sharedWith && sharedWith.length > 0) {
            const notificationPromises = sharedWith.map((friendId) => sendNotification(friendId, '🌦️ Weather Share', `${userName} is experiencing ${condition}`, { type: 'weather', userId }));
            await Promise.all(notificationPromises);
